...
from morphokin.data.trie import Trie, TrieNode
...
def next_syllable_options(dictionary:Trie, parent_node:TrieNode):
...
def next_syllable_options_s2s(dictionary:Trie, parent_node:TrieNode):
    if parent_node is None: # Non-word-state --> Return any char other than blank
...
def clean_lower_text(string, encoding="utf-8") -> str:
...
def text_to_id_sequence(text) -> List[int]:
    seq = []
    txt = clean_lower_text(text)
    txt = re.sub(r"\s+", '|', txt).strip()
    start = 0
    end = 0
    while(end < len(txt)):
        if(txt[end] in VOWELS) or (txt[end] == '|') or (txt[end] == '\''):
            if(end > start):
...
def syllable_vocab_size() -> int:
    return len(VOCAB_TOKENS)

def syllable_vocab_units() -> List[str]:
    return VOCAB_TOKENS
